...
void read_mm_banner(Stream& input, matrix_market_banner& banner) {
    ...
    if (banner.type != "complex" && banner.type != "real" && banner.type != "integer"
        && banner.type != "pattern")
...
void CSR::construct(const std::string& mtx_file) {
    ...
    if (!ifile) {
        throw std::runtime_error(std::string("unable to open file \"") + mtx_file
    ...
    if (num_entries_read != nnz)
        throw std::runtime_error("read nnz not equal to decalred nnz "
                                 + std::to_string(num_entries_read));
