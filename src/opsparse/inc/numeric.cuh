...
__global__ void __launch_bounds__(NUMERIC_PWARP_BLOCK_SIZE, 2)
    ...
    __shared__ mint shared_mem[NUMERIC_PWARP_ROWS * NUMERIC_PWARP_TSIZE
                               * (sizeof(mint) + sizeof(mdouble)) / sizeof(mint)];
    ...
    mdouble* mono_shared_val = (mdouble*) (shared_mem
                                           + NUMERIC_PWARP_ROWS * NUMERIC_PWARP_TSIZE);
    mint j, k;
    ...
    mdouble aval, bval;
    for (j = d_arpt[rid] + tid; j < d_arpt[rid + 1];
         j += NUMERIC_PWARP) { // pwarp per row, thread per a item, thread per b row
        acol = d_acol[j];
        aval = d_aval[j];
...
__global__ void __launch_bounds__(1024, 2)
    ...
    mint* table_col = d_tables
                      + blockIdx.x * max_tsize
                            * ((sizeof(mint) + sizeof(mdouble)) / sizeof(mint));
