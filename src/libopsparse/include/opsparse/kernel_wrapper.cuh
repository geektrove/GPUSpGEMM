...
long compute_flop(mint* row_pointer_A,
                  mint* col_index_A,
                  mint* row_pointer_B,
                  mint M,
                  mint* floprC) {
...
#pragma omp for
        for (mint i = 0; i < M; i++) {
            ...
            for (mint j = row_pointer_A[i]; j < row_pointer_A[i + 1]; j++) {
                local_sum += row_pointer_B[col_index_A[j] + 1]
                             - row_pointer_B[col_index_A[j]];
            }
            floprC[i] = local_sum;
...
void h_setup(const CSR& A, const CSR& B, CSR& C, Meta& meta, Timings& timing) {
    ...
    CHECK_ERROR(cudaMemcpy(meta.max_row_nnz,
                           C.d_rpt + C.M,
                           sizeof(mint),
...
inline void h_symbolic_binning(CSR& C, Meta& meta) {
    ...
    if (*meta.max_row_nnz <= 26) {
        ...
        k_symbolic_binning2<<<GS, BS, 0, meta.stream[0]>>>(C.d_rpt,
                                                           C.M,
                                                           meta.d_bins,
                                                           meta.d_bin_size,
                                                           meta.d_bin_offset);
...
void h_symbolic(const CSR& A, const CSR& B, CSR& C, Meta& meta) {
    ...
    if (meta.bin_size[7]) { // shared hash with fail
        ...
        CHECK_ERROR(cudaFuncSetAttribute(k_symbolic_max_shared_hash_tb_with_fail,
                                         ...
                                         98304));
        k_symbolic_max_shared_hash_tb_with_fail<<<meta.bin_size[7],
                                                  1024,
                                                  98304,
                                                  meta.stream[7]>>>(
            A.d_rpt,
    ...
    if (meta.bin_size[7]) {
        CHECK_ERROR(cudaMemcpyAsync(&fail_bin_size,
                                    d_fail_bin_size,
                                    sizeof(mint),
                                    cudaMemcpyDeviceToHost,
                                    meta.stream[7]));
...
inline void h_numeric_full_occu(const CSR& A, const CSR& B, CSR& C, Meta& meta) {
    ...
    if (meta.bin_size[6]) {
        CHECK_ERROR(cudaFuncSetAttribute(k_numeric_max_shared_hash_tb_half_occu,
                                         cudaFuncAttributeMaxDynamicSharedMemorySize,
                                         98304));
        k_numeric_max_shared_hash_tb_half_occu<<<meta.bin_size[6],
                                                 1024,
                                                 98304,
                                                 meta.stream[6]>>>(
            A.d_rpt,
    ...
    if (meta.bin_size[7]) { // global bin
        ...
        size_t global_size = meta.bin_size[7] * max_tsize
                             * (sizeof(mint) + sizeof(mdouble));
    ...
    if (meta.bin_size[5]) {
        k_numeric_shared_hash_tb_full_occu<4096, 1024>
            <<<meta.bin_size[5], 1024, 0, meta.stream[5]>>>(A.d_rpt,
                                                            A.d_col,
                                                            A.d_val,
                                                            B.d_rpt,
                                                            B.d_col,
    ...
    if (meta.bin_size[4]) {
        k_numeric_shared_hash_tb_full_occu<2048, 512>
            <<<meta.bin_size[4], 512, 0, meta.stream[4]>>>(A.d_rpt,
                                                           A.d_col,
                                                           A.d_val,
                                                           B.d_rpt,
                                                           B.d_col,
    ...
    if (meta.bin_size[3]) {
        k_numeric_shared_hash_tb_full_occu<1024, 256>
            <<<meta.bin_size[3], 256, 0, meta.stream[3]>>>(A.d_rpt,
                                                           A.d_col,
                                                           A.d_val,
                                                           B.d_rpt,
                                                           B.d_col,
    ...
    if (meta.bin_size[2]) {
        k_numeric_shared_hash_tb_full_occu<512, 128>
            <<<meta.bin_size[2], 128, 0, meta.stream[2]>>>(A.d_rpt,
                                                           A.d_col,
                                                           A.d_val,
                                                           B.d_rpt,
                                                           B.d_col,
    ...
    if (meta.bin_size[1]) {
        k_numeric_shared_hash_tb_full_occu<256, 64>
            <<<meta.bin_size[1], 64, 0, meta.stream[1]>>>(A.d_rpt,
                                                          A.d_col,
                                                          A.d_val,
                                                          B.d_rpt,
                                                          B.d_col,
