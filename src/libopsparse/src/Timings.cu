...
void Timings::print(double total_flop) {
    double total_flop_G = total_flop / 1'000'000'000;
    printf("total flop %lf\n", total_flop);
    double sum_total = setup + symbolic_binning + symbolic + numeric_binning + reduce
                       + prefix + allocate + numeric + cleanup;
    if (measure_separate) {
