...
void Meta::allocate(CSR& C) {
    ...
    cub::DeviceScan::ExclusiveSum(nullptr,
                                  ...
                                  C.d_rpt,
                                  C.d_rpt,
                                  M + 1); // calculate tmp_storage_size in bytes
...
void Meta::memset_all(mint stream_idx = 1) {
    CHECK_ERROR(
        cudaMemsetAsync(d_bin_size, 0, (NUM_BIN + 2) * sizeof(mint), stream[stream_idx]));
    //CHECK_ERROR(cudaMemset(d_bin_size, 0, (NUM_BIN + 5) * sizeof(mint)));
}

void Meta::memset_bin_size(mint stream_idx = 1) {
    CHECK_ERROR(
        cudaMemsetAsync(d_bin_size, 0, NUM_BIN * sizeof(mint), stream[stream_idx]));
    //CHECK_ERROR(cudaMemset(d_bin_size, 0, (NUM_BIN + 5) * sizeof(mint)));
}

void Meta::D2H_all(mint stream_idx = 0) {
    CHECK_ERROR(cudaMemcpyAsync(bin_size,
                                d_bin_size,
                                (NUM_BIN + 2) * sizeof(mint),
...
void Meta::H2D_bin_offset(mint stream_idx = 0) {
    CHECK_ERROR(cudaMemcpyAsync(d_bin_offset,
                                bin_offset,
                                NUM_BIN * sizeof(mint),
