...
// Inner implementation of the proposal sparse matrix multiplication algorithm
// This function orchestrates the phased execution of sparse matrix multiplication
// Template parameters:
// - T: The floating-point type (float or double)
// - Params: Configuration parameters for the algorithm specific to compute capability
...
auto proposal_inner(const utils::DeviceCSR<T>& A, const utils::DeviceCSR<T>& B)
    ...
    auto get_value_difference = [values = C.rpt] __device__(const std::int32_t row) {
        // Get NNZ of a specific row as a difference because C.rpt is already scanned (exclusive prefix sum)
...
// Main entry point for the proposal sparse matrix multiplication algorithm
// This function detects the GPU compute capability and dispatches to the appropriate
// specialized implementation with optimized parameters
...
auto proposal(const utils::DeviceCSR<T>& A, const utils::DeviceCSR<T>& B)
    ...
    // Detect the compute capability of the current GPU
    ...
    // Select the appropriate implementation based on compile-time flags and runtime detection
